...
- DSアーキテクチャとAPIアーキテクチャの両方の要素を含む、もっとも複雑なアーキテクチャ

## 5.1 ストリーミングアーキテクチャの紹介

- オフラインのバッチやAPIとは異なり、ストリーミングデータは、イベントとの相互作用や、イベントへの反応に重点を置いています。
- これにより、分散環境で、最新のマテリアライズドビューが実現できます。

★[疑問] どういう意味？？？

## 5.2 非同期イベントモデルがもたらすもの

- APIアーキテクチャとストリーミングアーキテクチャの大きな違いは、ストリーミングアーキテクチャの会話の方向が逆向きになること
- 非同期通信の良い例が電子メールです。送信ボタンを押したら、作業に戻るはずです。相手の返信を待つことはありません。
- イベントやメッセージをメッセージキューに送り、ほかのシステムがそれを取得するのを待つのです。
- このような疎結合により、ほかのパーティーが反応するのを待つという依存関係がなくなります。
- 最新のメッセージングプラットフォームでは、分散（ビッグデータ）機能とフォールトトレランス機能により、非同期モデルをスケールアップしている点が大きく異なります。
- 長期的な永続的ストレージも、ストリーミングプラットフォームが従来のキューイングシステムと異なっている点です。

## 5.3 イベント駆動型アーキテクチャとは

- イベント駆動型アーキテクチャは、サービス駆動ではなく、イベント駆動を中心に考えたアーキテクチャです
- イベントとは、「状態の変化」や「何かが発生した」と表現されるものです。例えば、新規顧客の登録や注文、飛行機の着陸などがこれにあたります。
- このパターンには、少しだけ設計思想の異なる参照モデルがあります。メディエータートポロジーとブローカートポロジーのいずれかが使用されます。

### 5.3.1 メディエータートポロジー

- イベント処理を管理するために、ある程度の中央コントロールや調整が必要な際に、アーキテクチャ設計で使われるのがメディエータートポロジーモデルです。
- このトポロジーは、イベントを処理するために複数の段階が必要な場合によく使用されます。
- トランザクションシステムのように、各メッセージが1 度だけ、または、特定の順序で処理されることが重要である場合に、一般的に使われるのがメッセージキューです。
- メディエータートポロジーをソフトウェアアーキテクチャで実現したものに、出版-購読型モデル（publish-subscribe model）があります

☆[確認] PubSubの話か

### 5.3.2 ブローカートポロジー

- ブローカートポロジーモデルもメッセージを使いますが、中央のメディエーターが存在しない点がメディエータートポロジーと異なっています。
- イベントの流れが比較的単純で、中央での調整やオーケストレーションが不要なシナリオの場合、このモデルが使われます。
- ブローカートポロジーモデルのソフトウェアアーキテクチャの実装は、メディエータートポロジーモデルと同じように、さまざまな方法で行うことができます。
  - Apache Kafka（https://kafka.apache.org）は人気があります。
  - RabbitMQ（https://www.rabbitmq.com）やActiveMQ（https://activemq.apache.org）のようなソリューションも人気です。

★[疑問] 以下に書いてあることとあまり一致しない。謎。
- [「ソフトウェアアーキテクチャの基礎」読書感想](https://zenn.dev/yoshiko/scraps/124fbe6d5b2103)

### 5.3.3 イベント処理スタイル

- EDAトポロジーモデルには、それぞれイベントを処理するためのスタイルがあります。
- シンプルイベント処理（SEP、Simple evnet processing）は、最もシンプルな処理スタイルです。ある特定の測定可能な状態の変化に対して、直接関連するイベントを処理します。
- イベントストリーム処理（ESP、Event stream processing）は、ストリームと複数のイベント（メッセージ）に対して、集約や、フィルタリング、結合などの複雑な処理を同時に適用するものです。
- 複合イベント処理（CEP、Complex event processing）は、イベント処理の中で最も複雑な形態です。この処理では、分析を行い、より複雑な事象が発生したかどうかを判断するために、パターンを見つけます。
- これから学ぶように、複合イベント処理は、一般的に必ずイベントストリーム処理と組み合わされます。
- イベント駆動型アーキテクチャの実装は複雑です。
- Apache Kafkaや、IBMMQ、Apache Pulsar、RabbitMQ、AWS SNS、AWS Kinesis、Azure Service Bus、Google Cloud Pub/Subなどのイベントストリーミングプラットフォームを配備するだけでは十分ではありません。
- EDAでは、イベントを作成、収集、変換、消費するために、多くのコンポーネントを追加しなければなりません。
- イベント駆動型アーキテクチャの仕組みを理解するために、その基盤となるテクノロジーの1 つであるKafka を深く掘り下げます。

## 5.4 Apache Kafka入門

- KafkaはもともとLinkedInが開発したものです。
- Kafka は、すべてのメッセージを分散ファイルシステム上の不変（追記のみ可）のファイルに格納するという独特なアーキテクチャを採用しています。
- Kafka は、保持期間（TTL、Time to Live）と呼ばれる時間パラメータとともにメッセージを保持します。保持期間は、一般的に、7 日間です。
- 保持期間が経過すると、メッセージは捨てられます。
- Kafka は、メッセージキューと同じように、トピックを使ってデータを整理します。
- 各トピックは、複数のデータコンシューマが関心を持つ可能性のある、データのグループです。
- データコンシューマは、トピックを購読します。みなさんがメーリングリストに登録するのと同じような感じです。
- データコンシューマは、オフセットを使ってトピックを読み出します。
- イベントやメッセージが処理された後、データコンシューマはKafka にコミットしなければなりません。コミットを行うと、その結果としてオフセットが増加します。
- Streams API を使用すると、高度なパターンを実装することができます。
- データがプラットフォームに入ってきたり、プッシュされたりしたときに、あるトピックから別のトピックへのイベントをインターセプトして操作することができます。

### 5.4.1 分散型イベントデータ

- Kafka の先進的な機能として、データの複製機能があります。
- Kafka のミラーリング機能により、複数の物理環境にまたがるクラスタ間でトピックを同期させることができる。

### 5.4.2 Apache Kafkaの特徴

- Apache Kafkaは、スケーラブルで耐障害性の高いストリーミングプラットフォームです。また、多くの機能を備えています。
- このプラットフォームを使えば、分散型アプリケーションの構築が可能になります。
- EDAアーキテクチャのコアコンポーネントとして多くの大企業で、Kafkaが使われているのには理由があります。
- Kafka は、洗練された流通プラットフォームであり、高度な統合機能とAPI を備えています。

## 5.5 ストリーミングアーキテクチャ

- イベント駆動型アーキテクチャは、一般に、3 つの構成要素からなります。イベントプロデューサーと、プラットフォーム、イベントコンシューマです。

### 5.5.1 イベントプロデューサー

- ソースシステムの多くは自分自身でデータを流したり、イベントを生成したりするわけではありません。
- フロントエンドとユーザーインタフェースイベント
  - 例えば、Webページを閲覧したり、商品をカートに入れたりするブラウザ内のイベントは、直接収集することができます
  - アプリケーションが3 層構造のソフトウェアアーキテクチャを使う場合、イベントはプレゼンテーション層から生成されます
- アプリケーションイベント
  - イベントや通知は、アプリケーション自身が発行することもできます。
  - この場合、ビジネスロジック層がそのようなイベントを生成する責任を負います。
- アプリケーションデータベースの読み出し
  - データストレージ層を読み出すことで、アプリケーションデータベースに加えられた変更を捕捉してイベントを発生させる方法もあります

☆[確認] 3層それぞれでイベントが発生する

- 一般的に、ポーリングベースの方法は、セットアップが簡単ですが、常にスキャンを行うため、データベースへの負荷が高くなる場合があります。
- また、ポーリングを正しく行うための対策を考える必要があるという欠点もあります。
  - 例えば、ある行がデータベースから取得しなければならないのかどうかを検出するために、「updated_at」カラムを追加しておかなければならない場合もあるでしょう。

- 状態を把握する方法として、ログベースや、CDC ベース、コンポーネントを使うという方法（2）もあります。
- これは、トランザクションログからデータベース内の1 つ1 つの変更を読み出すことで動作します。

- もう1つの方法（図5-6 の3）は、データベースをリッスンしたり、イベントを生成したりするためのフレームワークを自分で記述することです。
- コネクタやログベースのツールを使う代わりに、データベースやAPIを読み出したり、常時リッスンしたりする小さなアプリケーションコンポーネントを使ってイベントをインジェストします。

### 5.5.2 イベントコンシューマ

- 最もシンプルなユースケースは、メッセージブローカーを利用することで、イベントプロバイダーとイベントコンシューマを分離することです。
- 複数のイベントコンシューマが同じイベントストリームを消費する場合があります。
- このような場合、コンシューマはそれぞれ独自にメッセージオフセットを管理します†6。このパターンはファンアウト交換と呼ばれます。
- より複雑な状況としては、最新のアプリケーション機能で運用システムを拡張しなければならない場合です。
- このコンポーネントは別の場所で実行されており、イベントによって呼び出されます。そして、メッセージを処理し、結果を保存します。もしくは、別のデータベースを検索して、その結果を運用システムに送り返します。
- このパターンはストラングラーパターンとも呼ばれます
- 高度なシナリオとしては、Apache StormやApache Spark などのストリーム処理エンジンや分析プラットフォームを使用して、複数のイベントプロバイダーのイベントストリームを網羅的に分析し、パターンを探すシナリオが考えられます。
- データをデータベース内に格納する前にクエリを実行して分析するパターンは、複合イベント処理とも呼ばれます。

### 5.5.3 イベントプラットフォーム

- パブリッククラウドでは、主要ベンダーが同様のサービスを提供しています。
- AWS の場合は、DynamoDB（https://oreil.ly/xg5uz）（データベース）、SQS（Simple Queue Service）（https://oreil.ly/tkuV_）、Kinesis（https://oreil.ly/yMwsc）（イベントプロセッサ）を使えば、たいていのことはできるでしょう。

- これにより、いくつか高度なパターンを実装できます。1 つはイベント駆動型サービスコレオグラフィです。
- プロセスの更新やコマンドを非同期で流通させ、複数のアプリケーションが対応できるようにするものです。
- このほかに、イベントソーシングとコマンドソーシングという2 つのパターンも使えます。

### 5.5.4 イベントソーシングとコマンドソーシング

- イベントソーシング（https://oreil.ly/Tath1）では、アプリケーション状態やデータベースに対する変更が、すべて一連のイベントとして、不変のログや追記型データストアに保存されます。
- すべてのイベントが順番に格納されているため、アプリケーションデータベースのデータをほかの場所で完全に再構築したり、アプリケーションデータベースのデータをある場所から別の場所に複製したりすることもできます。
- もう一つのパターンとして、コマンドソーシングを実装することもできます。これは、コマンドをインターセプトすることで、イベントソーシングとは逆の方向に機能します。

### 5.5.5 ガバナンスモデル

- 1 つのクラスタをプライベート通信とパブリック通信の両方に使用すると、ややこしいことになります。
- ほかのドメインのコンシューマが、ほかのドメインのプライベート通信チャネルを拾ってしまってはならないからです。
- これを避けるために、プライベートストリームとパブリックストリームを強力に分離すること（図5-10）を推奨します†8。

- これに対して、パブリックストリームは、ほかのドメインで消費されます。パブリックストリームはビジネスストリームとも呼ばれます。
- ビジネスで消費されるものなので、読み出しに最適化されています。
- パブリックストリームは分離されているため互換性が保たれ、レジストリに登録され、データコンシューマのために詳細が提供されます。

### 5.5.6 ビジネスストリーム

- ドメインがほかのドメインとデータを交換したり、イベントを配信したりしたい場合、RDSやAPIアーキテクチャと同じポリシーに従わなければなりません

#### 5.5.6.1 アプリケーション境界内

- 最終的には、変換結果である非正規化テーブル（図5-11）がアプリケーションデータベースに格納され、このテーブルがストリームの入力となります。

#### 5.5.6.2 サードパーティー製ツールの使用

- 複雑なデータ構造を持ち、調整やアクセスが困難なアプリケーションでは、ビジネスイベントを生成するために、サードパーティー製のツール（図5-12）を使うと良いでしょう。

#### 5.5.6.3 インジェストの後処理

- 中央プラットフォームにインジェストした後に、技術データを変換する方法もあります。例えば、Kafka のKSQLを使えば、プラットフォーム内でデータをある構造から別の構造にリアルタイムで変換できます（図5-13）。

### 5.5.7 ストリーミングの消費パターン

- 消費側では、ニーズがより多様でユースケースに依存するため、さらに複雑さが増します。
- （消費する）ドメインの境界内で消費され、変換されるイベントは、ストリーミングプラットフォームと、ドメイン自体によって提供される追加のフレームワークやコンポーネントを利用することができます。
- イベントコンシューマがストリームデータを処理する方法について、いくつかのバリエーションを説明しましょう。

#### 5.5.7.1 プラットフォーム機能の利用

- 最初に紹介する消費パターンは、中央プラットフォームを直接使用してイベントを変換するものです。

#### 5.5.7.2 マイクロサービスとイベントフレームワーク

- マイクロサービス（3）を使って、イベントコンシューマのユースケースに合わせてイベントを変換し、リッチ化するという方法もあります。

#### 5.5.7.3 ストリーミング分析サービス

- 分析的なユースケース（複合イベント処理）を支援するために、ストリーミングコンポーネント（図5-14の4）を追加することができます。

#### 5.5.7.4 パブリッククラウド上でのイベントルーティング

- 最後に紹介するのは、AWS Kinesis やAzure Event Hubs（https://oreil.ly/ZaRye）などのツールを使って、パブリッククラウド上でイベントルーティング（転送）する方法です。

### 5.5.8 イベントによる状態転送

- 異なるドメインは分離すべきですが、ストリーミングアーキテクチャのストリーミングプラットフォーム（群）を使って、ドメイン内のアプリケーション間で状態を移動してもかまいません。
- イベントとその粒度を設計する場合、状態転送なのか状態通知なのかについて考えるようにしてください。
...
- 中央ストリーミングプラットフォームは、アプリケーションのバックエンドとして使用できるのでしょうか。
- 例えば、Kafka をアプリケーションのデータストアとして使うことはできるのでしょうか。
- 理論的には可能ですが、中央プラットフォームを、アプリケーションのバックエンドとして使用することはお勧めしません。

## 5.6 運用のバックボーンとしてのストリーミング

- 1 章では、トランザクションシステムと分析システムがより密接に連携していくトレンドについて説明しました。この問題を解決するのが、ストリーミングデータです。

## 5.7 保証と一貫性

- ストリーミング処理は、バッチ処理よりも複雑です。バッチ処理で何か問題が発生した場合、すべてのデータを再送信し、バッチ処理を再実行することで問題を解決することができます。

### 5.7.1 一貫性（整合性）レベル

### 5.7.2 「少なくとも1回、正確に1回、多くても1回」処理

- 結果整合性モデルと強一貫性モデルには、さまざまな処理モデルが存在します。少なくとも1 回処理は、メッセージが失われないことを保証します。
- 正確に1 回処理は、各イベントが正確に一度だけ配信されることを保証します。1 回のみ配信モデルとも呼ばれます。
- 最後に紹介するモデルは、多くても1 回処理です。無保証処理と呼ばれる場合もあります。このモデルでは、すべてのメッセージが正しく配送されることは保証されません。メッセージは失われてもよく、また、何度も配送される場合があります。
...
- 中立的な方法として、データ形式や型の検証を行えるように、インタフェース記述言語（https://oreil.ly/2CLeH）を使用して、メッセージをエンコードするようなプロトコルを使うのが一般的な選択肢となります。

## 5.8 ガバナンスとセルフサービスモデルのためのメタデータ

- ストリーミングアーキテクチャには、ストリーミング機能のほかに、メタデータ機能やメタデータ要件が付加されています。
- その中でも重要なのは、トピックレジストリ（スキーマレジストリ）です。
- これらのパターンの大半は、（中央）レジストリを持つプラットフォームを使う必要があります。

# 6章 すべてをまとめる

## 6.1 アーキテクチャの振り返り

- ドメインが作業を終え、データが統合アーキテクチャのいずれかを介して消費される準備が整うと、そのデータは誰でも利用できるようになります。

### 6.1.1 RDSアーキテクチャ

- RDSアーキテクチャは、主にビジネスインテリジェンスや分析など、より大量のデータを扱うユースケースに対応するものです。
- このアーキテクチャには、CQRSと多くの類似点があることがご理解いただけていると思います。

### 6.1.2 APIアーキテクチャ

- API アーキテクチャは、リアルタイムの同期通信に重点を置いたアーキテクチャです。
- また、クライアントサーバーアーキテクチャで一般的に使われているリクエストレスポンスモデルを使用します。
- RDSはもともと非同期なので、一貫性のある、正確なデータのためには、RDSアーキテクチャはうまく機能しません。
- このような場合は、APIアーキテクチャを使用する必要があります。

### 6.1.3 ストリーミングアーキテクチャ

- イベント駆動型通信のことです。イベント駆動型通信を通じてリアルタイムにデータを変換し、ほかのアプリケーションに通知することができます。
- イベントストリーミングは会話的ではなく、リスナーに対してデータを送る一方通行の通信路です。
- APIアーキテクチャとストリーミングアーキテクチャは、異なるユースケースに対して、根本的に異なるワークフローをサポートします。
- イベント駆動型アーキテクチャは、アプリケーションの状態を伝えるイベントを流通して、データ複製を実現することもできます。
- 本当に素晴らしいところは、すべてのドメインチームが独立して進化し、自分たちに最も適したアプリケーション統合パターンを選択できることです。

### 6.1.4 強化パターン

- いくつかのアーキテクチャを組み合わせることで互いを強化することができます。
- ゲートウェイルーティングとインデックステーブル
  - RDS とAPI を組み合わせて、クエリトラフィックをRDS へルーティングし、コマンドトラフィックを運用システムへルーティングすることができます。
- 分散型RDS
  - アプリケーションの状態をリアルタイムでRDSに複製することで、特にクエリの多いデータをデータコンシューマの近くに保存することができます。
  - これは特に、運用システムに対する重い読み出しの影響を緩和したり、ワークロードをクラウドに移行する際の帯域幅を削減したりするときに便利です。
- キューベースの負荷平準化
  - APIアーキテクチャとストリーミングアーキテクチャを組み合わせて、バッファとして機能するキューを作成し、APIアーキテクチャへのリクエストを保持するようにします。
- ノーティファイア
  - データが利用できるようになったことをコンシューマに知らせるために、RDSの通知サービスが必要になる場合があると述べました。
  - このサービスにより、チームはパイプラインを自動化することができます。
- ストリーミングインジェスト
  - ストリーミングプラットフォームを使って、アプリケーションの状態を転送することで、RDSを構築する方法です。
- 本章のアーキテクチャパターンは、通常、データレイヤーに適用されます。
- アプリケーションやシステム内において、提供側や消費側に適用されるソフトウェアアーキテクチャパターンは、ほかにも数多く存在します。
- アーキテクチャ内で適用できるパターンの包括的なリストについては、Microsoft のAzure Architecture Center（https://oreil.ly/S-9NU）を参照することをお勧めします。
...
- インタフェースの変更への対応や、ドメイン間での依存関係の管理、データへのアクセスとアドレス指定、ネットワーク環境全体でのデータ流通の方法などについて学びます。

### 6.2.1 安定したデータエンドポイント

- データプロバイダーとデータコンシューマは完全に自律していて、相手のことを意識しないですむようにするのが理想的な状態です。
- このような形式の「分離」を実現するためには、結合に関していくつかの原則を設定する必要があります。
- Scaled Architecture では、結合はデータレイヤーに存在します。
- データウェアハウスとは異なり、データプロバイダーのみがデータコンシューマと結合します。
- スキーマの進化は、データベース構造の変更に対処し、スキーマの互換性を保つプロセスです。
- 後方互換性(backward compatibility)
  - 新しいスキーマを使用するドメインが、以前のスキーマで作成されたデータを読めるようにすることを意味します。
- 前方互換性(forward compatibility)
  - 新しいスキーマで提供されたデータが、古いスキーマを使うドメインで利用できることを意味します。
- 完全互換性
  - スキーマが後方互換性と前方互換性を両方持っていることを意味します。
- プロトコルも変わる可能性があります。例えば、スキーマを記述するためのメタデータ構造や、プライバシーやセキュリティに必要な新しいメタデータ情報などが追加される場合があります。
- プロトコルに対しても、バージョン管理を行うことをお勧めします。
- つまり、インタフェースに反映されるバージョンは2 種類になります。インタフェース自体のバージョンと、インタフェースプロトコルのバージョンです。
- インタフェースが壊れるような変更を管理・防止する方法の1 つとして、データ配信契約を使用する方法があります。
- データ配信契約は、サービス契約とも呼ばれます。これらの契約は、安定したアーキテクチャのために極めて重要です。

### 6.2.2 データ配信契約

- データ配信契約は、Scaled Architecture の中でも非常に重要な点です。
- 最も基本的なレベルでは、すべてのインタフェースの契約をドキュメント化することをお勧めします。
- スキーマ（メッセージ形式とデータ）や、転送形態、アプリケーションとの関係などです。
- 検索性と再利用のために、これらの契約はメタデータリポジトリに保存する必要があります。

★[疑問] 非常に重要なのに、なにもHowが書いてないような…？

- アーキテクチャのRDS検証モデルを強化するために、ハンドシェイクサービスを実装することができます。
- ハンドシェイクサービスは、データ配信後に確認や検証を行い、エラーコードを返します。

☆[確認] Validationをメタデータリポジトリの情報を使って行うということ？

### 6.2.3 アクセス可能でアドレス可能なデータ

- データへのアクセスをさらに効率的にするには、すべてのURI（Uniform Resource Identifiers）†3を中央リポジトリや、Consul（https://www.consul.io）などの構成管理システムで公開することを検討してください。
- さらに分離を進めるために、ドメインネームシステム（DNS）（https://oreil.ly/xutxs）を使用すると、インタフェースを壊したり、ドメインのデータアクセスを妨げたりせずに、IP アドレスやアプリケーションの物理的な場所を変更することができます。

### 6.2.4 ネットワーク間通信の原則

- すべてのデータを中央に保存すると、大きな問題が発生します。同じデータを何度もコピーすると、ネットワーク帯域がすぐに足りなくなってしまいます。

#### 6.2.4.1 消費地に向けてデータを複製する

- この問題の解決策は、ステートフルな処理を消費が行われる場所に分散させることです。
- API ゲートウェイはそれぞれの側に1つしか存在していません。
- API アーキテクチャは主にリクエストレスポンスパターンという、同期的でステートレスな通信形態を用いるためです。
- 遅延と帯域幅の問題を解決するには、相手側のAPI ゲートウェイにキャッシュレイヤーを導入することをお勧めします。

#### 6.2.4.2 非同期の発想で考える

- ネットワークをまたぐ場合、別のリスクもあります。
- ネットワークに障害が発生すると、アプリケーションはそのデータにアクセスできなくなります。
- この問題を切り分け、より耐性のあるアーキテクチャにするためには、非同期の発想で考えはじめる必要があります。

#### 6.2.4.3 ドメインの分離

- 同期および非同期の分離は、ドメイン駆動設計（p.25「2.2.2 　ドメイン駆動設計」を参照）と強い関係があります。
- ネットワークやプラットフォームの接続性の問題に悩まされないために、可能な限り非同期通信にすることをお勧めします。
- また、原則として、コンテキスト境界を越える場合は、データレイヤーをその原則とともに使うようにしてください。

#### 6.2.4.4 例外的な場合

- ドメインやアプリケーションを越えて、データを流通させる場合、データレイヤーを使わない方が良い場合もあるのでしょうか。
- はい、ただし1 つだけです。唯一の例外は遅延が問題となるアプリケーションです。
- 例えば、銀行業界では、支払いをいかに早く処理するかが強く求められています。一般的に、処理時間の合計は100 ミリ秒以下でなければなりません。
- ポイントツーポイント接続はこの問題を解決します。しかし、メタデータのリネージは依然としてインサイトのために必要です。

#### 6.2.4.5 ハブスポークネットワークトポロジー

- より強く分離するための方法は、ネットワークによってドメインを分離するハブスポーク型ネットワークアーキテクチャを使う方法です。
- Azure VNet（https://oreil.ly/Vdl5B）や、AWS VPC（https://oreil.ly/4IxIq）、GCP VPC（https://oreil.ly/pSKo3）は、すべてクラウド上の隔離型ネットワークのオプションです。
- スポークを使用するドメインチームが、APIゲートウェイやストリーミングサービスなどの独自の統合コンポーネントを配備することを許可するかどうか、という設計上さらに難しい問題があります。

## 6.3 エンタープライズデータ標準

- 非中央集権型データアーキテクチャに移行するために重要なのは、連合型では、所有権は非中央集権的になることや、そのためには十分に知られた規律が必要であることを理解することです。

### 6.3.1 消費最適化の原則

- 繰り返し作業をなくすために、「データ提供は適切に一度、消費は何度も」という新しい標語を提案します。
- データは再利用可能な資産として提供され、同時にできるだけ多くの顧客に対応できるようにしなければなりません。ある特定のデータコンシューマのために設計してはなりません。
- 可読性と再利用性の目標は、データレイヤーを通過するデータセットの内部設計に影響を与えます。
- 例えば、延々と続く複雑な親子関係や、高度に正規化されたデータ構造、技術的なデータモデルなどは、データコンシューマにとっては、データが理解・利用しにくいものになってしまいます。
- 消費に最適化されたデータを流通するという方法は、多くのデータレイク参照アーキテクチャと矛盾しています。
- 複雑なデータを生のまま（1対1に）投入することは、近視眼的には利点があるかもしれませんが、長期的に見るとうまくいきません。
- 消費最適化されたデータを提供してください。つまり、過度に正規化された物理モデルや過度に技術的な物理モデルを、より再利用しやすく論理的にグループ化されたデータセットに変換する必要があります。
- このような処理をドメイン集約（https://oreil.ly/d0STt）と呼びます。
- データプロバイダーは自分のデータモデルをほかのドメインのニーズに合わせてはなりません
- データ要素は不可分でなければなりません。つまり、その属性はこれ以上意味のあるサブコンポーネントに分割できないようなものにしてください。
- データは、データセット全体で一貫した形式でなければなりません。これは、データ要素の表現形式や構文（小数部の精度、表記法、文法など）が同じであることを意味します。
- 消費側のドメインが正しい値を得るためにアプリケーションロジックを適用する必要がないようにしてください。
- 筆者のこれまでの経験によれば、適切なデータパイプラインとエンドポイントを構築することは、科学というより芸術です。
- データプロバイダーとデータコンシューマは、最適なデータ構造について意見を一致させなければなりません。
- アプリケーションで生データと消費最適化されたデータを同時に提供できるようにすることを検討してください。
- この方法は、データサイエンティストだけでなく、素早くデータにアクセスしたいコンシューマにもメリットがあります。
- 生データを提供するインタフェースは、プライベートにしておく必要があり、無保証としてください。本番用のデータパイプラインとして使うべきではありません。
- あるビジネスロジックや抽象化された機能をどこに配置するかを決めるのは、難しい場合があります。
- データプロバイダーとデータコンシューマのどちらにも明確に当てはまらない機能があるためです。
- データ消費に最適化しようとするドメインチームをサポートするために、設計の検討をガイドするコンピテンシーセンターを設置することをお勧めします。
- また、これらの指導活動では、わかりやすいドキュメントや、詳細なユースケース、説明、コード例などを提供する必要もあります。
- 例えば、Zalando のAPI ガイドラインドキュメント（https://oreil.ly/bqGm5）は、オープンソースの生きたドキュメントで、開発者を導き、共同作業を容易にしてくれます。
  - [Zalando RESTful API and Event Guidelines](https://opensource.zalando.com/restful-api-guidelines/)

### 6.3.2 メタデータの検索性

- 複数の統合パターンを使ってデータを同時に配信する場合、データに対する責任を負うのはデータ所有者のまま。
- データ所有者はゴールデンソース、ゴールデンデータセット、ゴールデンデータ要素と関係付けられています。
- なぜ所有権情報を抽象化するのでしょうか。なぜ、物理データ属性に直接関係付けないのでしょうか。それは、密結合を避けるためです。

### 6.3.3 意味的一貫性

- 唯一の違いは、データレイヤーを介して公開されるドメインデータは表現形式が異なり、通常はソースシステムやアプリケーションデータのサブセットとなることです。
- 論理モデルは、一般的に、関係データベーステクノロジーのように、アプリケーションに依存しないデータベース設計を行う場合にのみ意味を持ちます。
- しかし、今日のモデルの大半は、NoSQLスキーマ設計または非正規化データ構造を使用しているため、論理モデリングは逆効果になりつつあります。
- そのため、実際には、多くのエンジニアが論理モデリングのステップをスキップしています。
- 筆者が推奨する解決策は、概念データモデルから、すべてのビジネス概念と関係をオンラインオントロジーアプリケーションに中央集権的に保存し、
- それらをゴールデンデータセットの要素（別のリポジトリに保存可能）に関連付け、最後にインタフェースモデルの（スキーマ）メタデータに対して検証を行う方法です。
- コントロールとデータガバナンスの適用レベルに基づいて、データレイヤーへのインタフェースのカバー率を検証することができます。
- インタフェースのメタデータ属性のいずれもがゴールデンデータセットの要素とマッチングしない場合は、データプロバイダーによる、そのようなインタフェースの公開を停止することを検討してください。

### 6.3.4 対応するメタデータの提供

- データを検索しやすくし、他者が使用・再利用できるようにし、適切な制限をかけるには、データの中にメタデータを埋め込むことが必要です。

### 6.3.5 データの出所と移動

- リネージは、データガバナンスの中でも非常に重要です。データがどこで生まれ、どのように収集され、どのように修正され、どのように下流で消費されたかを把握するための「銀の弾丸」なのです。

★[疑問] 銀の弾丸とか絶対ないと思うがな

- 適切に設計されていれば、すべてのアーキテクチャの統合コンポーネントは、自動的にリネージをインジェストすることができます。

## 6.4 参照アーキテクチャ

- 前の章で紹介した3 つの統合アーキテクチャは、それぞれ独立しているのではなく、補完し合いながら密接に連携しています。
- データコンシューマのためのソリューションであるDDSがあります。これらはユースケースに基づいて構成されます。
- 運用や分析の両方で使用できます。ユースケースはさまざまであり、カスタマイズされるため、データモデルも同じように専用のものになります。
- 統合のパターンや手順も、リアルタイムのリクエストとレスポンスという単一のパターンから、データクレンジングやリッチ化などを含む複数のETL手順まで、さまざまなものがあります。
- メタデータレイヤーは、メタデータに依存するすべての構成要素を保持する抽象的なレイヤーです。
- データウェアハウスとデータレイクは、このアーキテクチャのどこに位置づけられるのでしょうか。
- データウェアハウスとデータレイクは、データプロバイダーとしてもデータコンシューマとしても、どちらの側にも位置することができます。

## 6.5 まとめ

- Scaled Architecture は、3 つの統合アーキテクチャをパッケージ化しているため、ほかの多くのアーキテクチャとは異なります。
- テクノロジーにとらわれず、包括的で、将来を見据えたものであり、どのようなタイプのアプリケーションも排除しません。
- Kubernetes、Hadoop、Kafka を使ったマイクロサービスだけを推奨しているわけではありません。
- 結合の悩みから開放されたので、メタデータがすべてをまとめる接着剤になります。
